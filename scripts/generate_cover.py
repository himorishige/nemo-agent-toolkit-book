...
def draw_title(draw: ImageDraw.ImageDraw) -> None:
    ...
    """
    # 上段（補助）: 33pt + 28pt
    sub_top_font = _font(34)
    sub_top2_font = _font(28)
    # 主役: 56pt
    hero_font = _font(58)
    # 下段（補助）: 30pt
    bottom_font = _font(30)

    lines: list[tuple[str, ImageFont.FreeTypeFont, tuple[int, int, int], int]] = [
        ("NIM + Docker", sub_top_font, WHITE, 0),
        ("ではじめる", sub_top2_font, MUTED, 8),
        ("NeMo Agent", hero_font, WHITE, 32),
        ("Toolkit", hero_font, WHITE, 6),
        ("ハンズオン", bottom_font, WHITE, 28),
    ]
